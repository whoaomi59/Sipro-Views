...
const Results = ({ projects }) => {
  const [selectedItems, setSelectedProyectos] = useState([]);
  ...
  const handleSelectAllProyectos = (event) => {
  ...
  const handleSelectOneProject = (_event, projectId) => {
    if (!selectedItems.includes(projectId)) {
      setSelectedProyectos((prevSelected) => [...prevSelected, projectId]);
    } else {
      setSelectedProyectos((prevSelected) =>
  ...
  const filteredProyectos = applyFilters(projects, query, filters);
  const paginatedProyectos = applyPagination(filteredProyectos, page, limit);
  const selectedBulkActions = selectedItems.length > 0;
  const selectedSomeProyectos =
    selectedItems.length > 0 && selectedItems.length < projects.length;
  const selectedAllProyectos = selectedItems.length === projects.length;
          ...
          {!selectedBulkActions && (
            ...
            >
              <Box>
                ...
                <b>{paginatedProyectos.length}</b> <b>{t('projects')}</b>
              </Box>
              <TablePagination
                component="div"
                count={filteredProyectos.length}
            ...
            <>
              <TableContainer>
                <Table>
                  <TableHead>
                    <TableRow>
                      <TableCell padding="checkbox">
                        <Checkbox
                          checked={selectedAllProyectos}
                          indeterminate={selectedSomeProyectos}
                          onChange={handleSelectAllProyectos}
                        />
                      </TableCell>
                  ...
                  <TableBody>
                    {paginatedProyectos.map((project) => {
            ...
            <>
              ...
              >
                <Box>
                  ...
                  <b>{limit}</b> {t('of')} <b>{filteredProyectos.length}</b>{' '}
                  <b>{t('projects')}</b>
                </Box>
                <TablePagination
                  component="div"
